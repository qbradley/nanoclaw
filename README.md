...
**Best harness, best model.** NanoClaw runs on the [GitHub Copilot SDK](https://github.com/github/copilot-sdk), with GitHub Copilot or BYOK (Anthropic, OpenAI) as the backend. The harness matters. A bad harness makes even smart models seem dumb, a good harness gives them superpowers.
...
**Session Management**
- `/clear` - Add a `/clear` command that compacts the conversation (summarizes context while preserving critical information in the same session). Requires figuring out how to trigger compaction programmatically via the Copilot SDK.
